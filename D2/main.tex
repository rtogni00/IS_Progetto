...
\begin{tabular}{lllllll}
	...
	BasicUser       & register()                           & POST        & /api/v1/users/register            & JSON {email, password}         & JSON {token, userData}           & Crea un nuovo utente \\
	...
	& getSavedEvents()                     & GET         & /api/v1/users/savedEvents         & Header {Authorization: Token}  & JSON {savedEvents}               & Ottiene gli eventi salvati \\
	& saveEvent(eventId)                   & POST        & /api/v1/users/save/{eventId}      & Header {Authorization: Token}  & JSON {message}                   & Salva un evento nei preferiti \\
